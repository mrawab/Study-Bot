...
def save_flashcard(message, question):
    answer = message.text
    if message.chat.id not in flashcards:
        flashcards[message.chat.id] = []
    
    flashcards[message.chat.id].append({"question": question, "answer": answer})
...
def view_flashcards(message):
    user_flashcards = flashcards.get(message.chat.id, [])
...
def process_flashcard_deletion(message):
    try:
        ...
        if index < 0 or index >= len(flashcards[message.chat.id]):
            raise IndexError
        flashcards[message.chat.id].pop(index)
