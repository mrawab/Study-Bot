...
def handle_file(message):
    if message.document.mime_type == 'application/octet-stream' or message.document.mime_type == 'application/x-ipynb+json':
        file_id = message.document.file_id
        file_info = bot.get_file(file_id)
        downloaded_file = bot.download_file(file_info.file_path)

        # Get the original file name and create the paths
        notebook_name = message.document.file_name
        notebook_base_name = os.path.splitext(notebook_name)[0]  # Remove the .ipynb extension
        ...
        bot.send_message(message.chat.id, "Conversion complete! Here's the Python script.")
